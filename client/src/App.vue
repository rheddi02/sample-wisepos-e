...
const cancelAction = async () => {
  const response = await fetch('/api/readers/cancel-payment-action', {
    method:'post',
    headers: {'Content-Type':'application/json'},
  ...
  addMessage(`Cancelled reader action on ${reader.value.label} ${reader.value.id}`)
  reset()
};
...
const simulatePayment = async () => {
  ...
  addMessage(`Simulating card reader ${reader.value.id} for payment`)
};
...
const capturePayment = async () => {
  ...
  addMessage(`Captured payment for ${paymentIntent.value.id}`)
  reset()
};
